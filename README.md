...
When you create a new tab, NppSync saves it to your shared folder with a generated two-word name (e.g. `fuzzy-lobster.txt`). The plugin watches the folder for changes from other machines and silently keeps all open notes in sync.
...
1. Download the latest release from the [Releases](https://github.com/belarrem/NppSync/releases) page
2. Copy the correct DLL for your Notepad++ version into its plugins folder:

   ```
   # 64-bit Notepad++
   C:\Program Files\Notepad++\plugins\NppSync\NppSync.dll

   # 32-bit Notepad++
   C:\Program Files (x86)\Notepad++\plugins\NppSync\NppSync.dll
   ```

3. Restart Notepad++
4. Go to **Plugins > NppSync > Settings** and set the sync folder path (e.g. `G:\My Drive\NppSync`)
...
| Delete a note everywhere | `Ctrl+Shift+D` or **Plugins > NppSync > Delete This Note Everywhere** |
| Rename a note | **Plugins > NppSync > Rename This Note** |
| Open the sync folder | **Plugins > NppSync > Open Sync Folder** |
| Restore dismissed notes | **Plugins > NppSync > Restore All Dismissed Notes** |
...
NppSync targets .NET Framework 4.8 and requires Visual Studio 2022 (or Build Tools). Use `MSBuild.exe` directly — `dotnet build` is not supported.

```powershell
$msbuild = "C:\Program Files\Microsoft Visual Studio\2022\Community\MSBuild\Current\Bin\MSBuild.exe"

# x64
& $msbuild NppSync.csproj /t:Restore /p:Platform=x64
& $msbuild NppSync.csproj /p:Configuration=Release /p:Platform=x64

# x86
& $msbuild NppSync.csproj /t:Restore /p:Platform=x86
& $msbuild NppSync.csproj /p:Configuration=Release /p:Platform=x86
```

Output: `bin\Release-x64\NppSync.dll` (64-bit) and `bin\Release\NppSync.dll` (32-bit)
...
You can access the settings from the Notepad++ menu > Plugins > NppSync > Settings
Settings are stored in `%APPDATA%\Notepad++\plugins\Config\NppSync\NppSync.ini`.
