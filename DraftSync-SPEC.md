# DraftSync — Notepad++ Cross-Machine Tab Sync Plugin
...
DraftSync is a Notepad++ plugin (.NET/C#) that syncs unsaved scratch tabs across multiple machines via a shared folder (Google Drive). When a user creates a new tab on any machine, it's assigned a fun two-word name (e.g., "fuzzy-lobster"), auto-saved to a shared folder, and automatically appears as a tab on all other machines running Notepad++ with the plugin.
...
- **Plugin DLL Name**: `DraftSync.dll`
- **Plugin folder**: `<Npp Install>/plugins/DraftSync/DraftSync.dll`
...
```
<SyncFolder>/                         # e.g., G:\My Drive\DraftSync\
├── fuzzy-lobster.txt                 # Note content (plain text)
├── turbo-pancake.txt
├── quantum-donut.txt
└── .draftsync/                         # Metadata folder (created with Hidden file attribute)
    ├── fuzzy-lobster.meta.json       # Per-note metadata
    ├── turbo-pancake.meta.json
    └── config.json                   # Shared config (future use)
```

The `.draftsync/` directory must be created with `FileAttributes.Hidden` so it doesn't clutter the sync folder in Explorer or the Google Drive web UI.
...
6. Creates `<SyncFolder>/.draftsync/cosmic-waffle.meta.json`
...
2. If it's a managed DraftSync tab:
   ...
   c. Show a custom DraftSync dialog:
   ...
   d. **Dismiss**: 
      ...
      - Add the note name to a local ignore list (`<Npp Plugin Config>/DraftSync.dismissed.json`)
      - File remains in sync folder — other machines are unaffected
      - Note will NOT reopen on next startup (ignored during scan)
      - Can be recovered via `Plugins > DraftSync > Show Dismissed Notes` (for selective recovery) or `Plugins > DraftSync > Restore All Dismissed Notes` (which clears the entire dismissed list and re-scans — **warning: reopens all previously dismissed notes**)
...
**Dismissed notes file** (`DraftSync.dismissed.json`, local only, not synced):
```json
{
  "dismissed": [
...
3. If the path has changed to a location **outside** the sync folder:
   ...
   c. Delete the corresponding `<name>.meta.json` from `.draftsync/`
...
3. Load the local dismissed notes list (`DraftSync.dismissed.json`)
...
Settings stored in `<Npp Plugin Config>/DraftSync.ini`:

```ini
[DraftSync]
SyncFolder=G:\My Drive\DraftSync
...
A simple WinForms dialog accessible via `Plugins > DraftSync > Settings`:
...
- Show a notification bar or message box: "DraftSync sync folder not found. Please configure in Settings."
...
```bash
# From solution root
msbuild DraftSync.sln /p:Configuration=Release /p:Platform=x64
msbuild DraftSync.sln /p:Configuration=Release /p:Platform=x86
```

Output:
- `bin/Release/x64/DraftSync.dll`
- `bin/Release/x86/DraftSync.dll`

### Install

Copy to Notepad++ plugins folder:
```
C:\Program Files\Notepad++\plugins\DraftSync\DraftSync.dll       # x64
C:\Program Files (x86)\Notepad++\plugins\DraftSync\DraftSync.dll # x86
```

### First Run

1. Launch Notepad++
2. `Plugins > DraftSync > Settings`
3. Set sync folder to Google Drive path (e.g., `G:\My Drive\DraftSync`)
4. Click OK
5. Plugin creates `.draftsync/` metadata subfolder
6. Start creating notes with `Ctrl+Shift+N` or `Plugins > DraftSync > New Synced Note`
